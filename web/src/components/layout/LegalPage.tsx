    ...
    <div className="container-pb grid gap-12 pb-24 pt-14 lg:grid-cols-[240px_1fr]">
      <aside className="lg:sticky lg:top-[calc(var(--header-h)+2rem)] lg:self-start">
        ...
        <nav className="flex flex-wrap gap-2 lg:flex-col lg:gap-1">
          {sections.map((s) => (
            <a key={s.id} href={`#${s.id}`} className="rounded-lg px-2 py-1 text-sm text-muted hover:bg-card-2 hover:text-ink">{s.title}</a>
          ))}
        </nav>
      ...
      <article className="max-w-3xl">
        ...
        <p className="mt-6 rounded-xl border border-gold/40 bg-gold/10 p-4 text-sm text-gold-soft">
